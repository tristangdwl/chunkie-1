function [val,grad,hess,der3,der4,der5] = helmdkdiffgreen(k,src,targ,ifr2logr)
...
logr = log(rnot);
g0(~isus) = io4*dh0da - r2fac*rnot.*rnot.*logr;
g1(~isus) = -k*io4*dh1da - io4*h1/k/2 - r2fac*(rnot+2*rnot.*logr);
g21(~isus) = -k*k*io4*dh0da - io4*h0 + k*io4*dh1da.*rm1 + io4*h1.*rm1/2/k - r2fac*(3+2*logr) - ...
    g1(~isus).*rm1;
g3(~isus) = k*k*io4*dh0da.*rm1 + io4*h0.*rm1 + io4*(k + (k*k-2*rm2)/k/2).*h1 + io4*k*(k*k-2*rm2).*dh1da - r2fac*2*rm1;
g4(~isus) = (io4*(3*rm2-k*k)/2/k - k*io4).*(2*h1.*rm1-k*h0) + k*io4*(3*rm2-k*k).*(2*dh1da.*rm1-k*dh0da - h0/2/k) + r2fac*2*rm2;
g5(~isus) = io4*(12*rm3-4*k^2*rm1).*h0 + io4*k*(12*k*rm3-2*k^3*rm1).*dh0da  + ...
    io4*((-24*rm4+7*k*k*rm2-k^4)/2/k+k*(7*rm2-2*k^2)).*h1 + io4*k*(-24*rm4+7*k*k*rm2-k^4).*dh1da- r2fac*4*rm3;
